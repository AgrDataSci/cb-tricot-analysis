...
The lecture on this topic is available [here]()
...
Load beans data from PlackettLuce, a data frame with 842 entries and 14 variables
...
With the ordering we can run the function `as.rankings(..., input = "orderings")` from PlackettLuce
...
Use these names to decode the orderings of the three varieties and create a [Sparse matrix](https://en.wikipedia.org/wiki/Sparse_matrix). Where each variety is a column, each row is the evaluation from a given farmer. Varieties not assigned by a given the farmer are set with 0.
